...
> Keep it noted as a open point pending on my side. This is hghest risk show stopper for GTM.
...
> remove  the words "for 200ms". The new line becomes "compiles in RAM and immediately replaces your text"
...
> Add it to list of job to be done. I'll do it, not an issue. I just have to open access to the git repo to all.
...
> The list needs to be updated with URL of all the 18 providers which are supported
...
> Update it to the list of 18 providers. Give the UI treatment same as present 3. 
...
> 10 is the number of prompts that user can change in ghost mode, after that user has to sign in. Once user has signed in, user can get prompts converted unlimited times. There is no concept of credits. What am I missing here from legal POV or are we good with this explanation?
...
> Put an asterisk over all. In the footnote on landing page we will say Live on chrome currently. Support to major browsers is underway.
...
> Change "completely inert in memory" to "No action". Does this change help us be 100% legally compliant? In letter as well as spirit?
...
> Keep it noted. It is a high priority point to be solved for, but not a showstopper
...
> "ChatGPT, Claude, Gemini, Microsoft Copilot, Perplexity, and  
> more" This is true and supports all scenarios. Use this in adaptive and compliant manner at all places.
...
> The idea is that 10 transforms per day as ghost, after that user has to sign up/in. This is the core idea and it has to come out in words. "10 transforms a day in ghost mode, after that, sign-up and continue". There is no concept of payment anywhere. The word "free" needs to be deleted. 
...
> Needs to be deleted for signed in user. Its irrelevant for signed in user.
...
> Make a note of this point. We will revisit it later, but this is not a show stopper.
...
> Make a note of it. It needs to be fixed. It is still not a show stopper. On day 1 itself I am not expecting a torrent of users.

### 3.2 No licence and no repository link

**S2.** Pillar 03 makes an open-source claim with nothing to click.

> Add link to patkan public github repository
...
> Give me revised and compliant copy for this piece "No obfuscated binary blobs. Because you install Patkan unpacked, you can open the folder, inspect every line of plain JavaScript, and verify every network call in DevTools before you click load." Keep word count +-5%.
...
> For now, remove library from signed in section. When a user signs in there should be a single message on screen saying "Thanks for signing-in. There is nothing more to do here. Get your best work done :)" There should be a CTA button with CTA text "Close", which takes user to patkan homepage, in signed in state. user can then choose to close the window or leave it open.
...
> Propose me copy for failure scenarios. Keep it very short, crisp and most importantly "human".
...
> Where do you propose we disclose this information? Give me 3 options which are consistent with brand promise and brand architecture pillars and yet, in full alignment with FOSS and data privacy principles.
...
> Hide all the tabs which are not live in code today. Make a note of it. It is a job to be done, to increase list of supporting browsers. 
...
> Fix this in code. user should see a reducing counter with the message "<no of count> more transforms, before you need to sign-in"
...
> Give me 3 options for a graceful and professional handling. Keep customer first and brand ethos and brand promise right next to it while you suggest the handling options.
...
> Give me 2 options for a graceful and professional handling. Keep customer first and brand ethos and brand promise right next to it while you suggest the handling options.
...
> "Other" should lead to a free text input field there itself, where user can input the persona. patkan should show that in the chip scroll list next time.
...
>  Give me 3 options on how to handle this, keeping in mind the brand ethos of the word "patkan". For user everything has to be patkan i.e fast, prompt, swift.
...
> How do we fix this? What is the solution? Propsoe the solution first. Dont fix it directly.
...
> Replce it with some very very popular or mandatory Chrome extension. We dont want to use grammarly.  
...
> What do you mean by that? restart of what? the claude/gemini session or opening the chrome browser every time? Does chrome ask every time to do something to developer mode to get extensions to work? I am not quite clear here.
...
> Remove mention of versions from all places. It has no significance.
...
> Mark it as a job to be done. To generate the social sharing image and plug it in all relevant places.
...
> [patkan.in](http://patkan.in) and [www.patkan.in](http://www.patkan.in) are live.
...
> I understand. Note that there is no subscription model, no data usage. This is a FOSS product I have made. Keeping that in mind propose how to handle this. 
...
> I have answered everything above. I want a fresh and updated list of job to be done for tasks that are to be done, and fix the items for which I have indicated what is to be done. Propose me options where I have asked for proposal from your side so we can close them. DO NOT leave anything from this list unaddressed.